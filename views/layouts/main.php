...
if(!Yii::$app->user->isGuest){
    $roles = (new \yii\db\Query())
        ->select("*")
        ->from('roles')->where(["id"=>Yii::$app->user->identity->role_id])
        ->one();

    foreach (json_decode($roles['actions'],1) as $key=>$action){

        if(stripos($key,Yii::$app->controller->id)===0){
            if(!isset($action[Yii::$app->controller->action->id])){
